...
import style from "./Koran.module.css";
...
export default function Koran() {
  ...
  return (
    <>
      <h1 className="text-center awgat text-4xl font-bold text-emerald-600 pt-20 pb-8">
        القرأن الكريم
  ...
  {koran.length > 0 ? (
        <div className="container awgat">
          <div className="row ">
            {koran.map((kor) => (
              <Link to={`/surah/${kor.number}/${kor.englishName}`} className="w-full md:w-1/5 lg:w-1/3  " key={kor.number} >
